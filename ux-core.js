...
const pal=['accent-4','accent-5','accent-6','accent-1','accent-2','accent-3'];cards.forEach((c,i)=>{let b=document.createElement('button');b.type='button';b.className=`tool-card ${pal[i%6]} enhanced-card`;b.dataset.extra=c[0];b.dataset.category=c[6].toLowerCase();b.dataset.search=c[7];b.innerHTML=`<span class="tool-icon">${c[1]}</span><b data-ar="${esc(c[2])}" data-en="${esc(c[3])}">${esc(c[2])}</b><small data-ar="${esc(c[4])}" data-en="${esc(c[5])}">${esc(c[4])}</small><em>${c[6]}</em>`;grid.append(b)});
