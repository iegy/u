...
- `ux-persist.js` — local supplier persistence and PWA install affordance.
- `sw.js` — offline cache.
...
The current application has no account system and no server-side database. Local drafts and supplier comparisons are stored in the user's browser. Clearing browser storage removes those saved drafts.
